...
import com.sys.mapper.apply.ApplyNsMapper;
...
public class MyTaskService {
                ...
                if(taskDefinitionList==null || taskDefinitionList.size()==0){
                   ...
                   // applyChange.setAcSqid(approve.getAplid());
                    ...
                    if(approve.getAptype().indexOf("ns")!=-1){
                        ...
                        String ysapplyid = this.applyNsMapper.selectById(approve.getAplid()).getApSqid();
                        ...
                        newContractDetail.setcHtlx(approve.getAptype().substring(approve.getAptype().indexOf("ns")+2));//设置申请类别
                        /*判断该条年审单对应的初审单是否是老数据*/
                        ...
                        if(!approve.getAptype().equals(PropertiesUtil.getApplyTypeProperties("nsbutie"))){
                            ...
                            applyChange.setAcApplyType(approve.getAptype().substring(approve.getAptype().indexOf("ns")+2));
                           ...
                           // String time=SendPhoneCodeUtil.getCron( DatetimeUtils.string2date("2018-12-20 14:40:00","yyyy-MM-dd hh:mm:ss"));
                           //String time = SendPhoneCodeUtil.getCron( DatetimeUtils.string2date(calendar.getTime(),"yyyy-MM-dd hh:mm:ss"));
                            ...
                            try {
                                ...
                                contractTemplateFillingDataPojo.setUserName(applyUserinfo.getUsername());//设置用户姓名
                                contractTemplateFillingDataPojo.setApSqlb(approve.getAptype().substring(approve.getAptype().indexOf("ns")+2));
                                tempMap.put("pageLaseIndex",5);
                                tempMap.put("pageIndex",0);
                                List<Map> approveList = this.approveMapper.findAllApply(tempMap);
                                contractTemplateFillingDataPojo.setObjSqbh(((Approve)approveList.get(0)).getF_community());//小区
                                contractTemplateFillingDataPojo.setBuilding(((Approve)approveList.get(0)).getF_buname());//栋
                                contractTemplateFillingDataPojo.setCtCode(((Approve)approveList.get(0)).getF_cecode());//单元
                                contractTemplateFillingDataPojo.setRoom(((Approve)approveList.get(0)).getF_buname());//房号
                                /*设置合同到期年月日*/
                                Calendar newCalendar = Calendar.getInstance();
                                newCalendar.setTime(endTime);
                                contractTemplateFillingDataPojo.setYearEnd(""+newCalendar.get(Calendar.YEAR));//年
                                contractTemplateFillingDataPojo.setMonthEnd(""+newCalendar.get(Calendar.MONTH));//月
    ...
    private void sendGzfNsMessage(ApplyUserinfo applyUserinfo,UserInfo userInfo,Approve approve,String tplid){
        ...
        String srouceinfo = new StringBuffer().append(tempApprove.getF_community())
                ...
                .append("单元").append(tempApprove.getF_buname())
                .append("室").toString();
    ...
    private void  addSignData(Approve approve,HttpServletRequest request){
        ...
        /*uuid命名签名图片名*/
        String fileName = CommonUtils.getUUIDWith_()+".jpg";
        ...
        Base64Util.base64ToImage(imgBase64Str,filePath,fileName); //生成签名图片

    }

    public static void main(String[] args){
        String testStr = "ns1";
        System.out.println(testStr.substring(testStr.indexOf("ns")+2));
    }
}