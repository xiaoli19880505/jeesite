...
public class QxInfoController {
    ...
    public  Object findAllQxInfo(String page,String rows,String mid){
        ...
        map.put("rows",rows);
        map.put("mid",mid);
        DataGridResult dataGridResult=this.qxInfoService.findAllQxInfo(map);
    ...
    public  Object deleteQxInfo(String qxid){
        String result="";
        Integer delete=this.qxInfoService.delete(qxid);
        if (delete>0){
            result="删除成功";
        }else {
            result="删除失败";
    ...
    public  Object updateQxInfo(QxInfo qxInfo){
        String result="";
        Integer updatecount=this.qxInfoService.update(qxInfo);
        if (updatecount>0){
            result="修改成功";
        }else{
            result="修改失败";
    ...
    public  Object insertQxInfo(QxInfo qxInfo){
        String result="";
        Integer count=this.qxInfoService.findQxinfoByNameOrMid(qxInfo.getQxName());
        if (count>0){
            result="模块下面有相同名称";
            return result;
        }
        qxInfo.setQxId(CommonUtils.getUUIDWith_());
        Integer updatecount=this.qxInfoService.insert(qxInfo);
